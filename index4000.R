...
check.csv <- function(csv.path){ #checks your criteria
  ...
  #test criteria
  meets.criteria <- ifelse(acc_max >= 4000, TRUE, FALSE)
...
Files_more_than_4000<-as.data.table(files.to.write)

write.csv(Files_more_than_4000, "/scratch/groups/jzeitzer/UKBB/Data/Index/Files_more_than_40000.csv")
